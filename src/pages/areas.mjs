...
<nav class="area-lines area-lines--detailed" aria-label="Areas we serve"><a href="/"><span>Kensington</span><small>W8</small><em>Stucco houses, garden squares, mews and mansion blocks.</em></a>${cfg.areas.map(a=>`<a href="/areas/${a.slug}/"><span>${a.name}</span><small>${a.postcodes}</small><em>${places[a.slug]}</em></a>`).join('')}</nav>
</div></section>`; }
