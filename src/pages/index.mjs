...
<nav class="map-area-list" aria-label="Areas we serve"><p class="map-area-list__label">Areas we serve · We're here</p>${areas.map(([, name, post, description, link]) => `<div class="map-area-list__item"><a href="${link}"><span>${esc(name)}</span><small>${esc(post)}</small><span aria-hidden="true">↗</span></a><p>${esc(description)}</p></div>`).join('')}</nav>
